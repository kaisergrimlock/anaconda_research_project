...
OUTPUT_DIR = Path(f"retrieved/trec_dl_{TRECDL_YEAR}/{TARGET_LANG}_word_sentence/")    # write mirrored CSVs
...
# ---------- Injection helpers ----------
def find_between_sentence_positions(text: str) -> List[int]:
    """
    Return insertion indices such that inserting at that index places content
    between sentences (i.e., right after '.', '?', '!' and after any following spaces/newlines).
    """
    positions: List[int] = []
    n = len(text)
    ...
    for i, ch in enumerate(text):
        if ch in ".?!":
            ...
            while j < n and text[j].isspace():
                j += 1
            # ensure there's content after the boundary
            if j < n:
                positions.append(j)
...
def inject_once(text: str, snippet: str) -> str:
    spots = find_between_sentence_positions(text)
    if not spots:
        return text
