...
TARGET_LANG = "eng"          # e.g., 'vi' for Vietnamese; 'eng'/'en' => no translation
SEED = 42                   # set None for non-deterministic injection
INJECT_COUNT = 4          # how many times to inject the translated query
...
OUTPUT_DIR = Path(f"retrieved/trec_dl_{TRECDL_YEAR}/{TARGET_LANG}_word_mult_3/")    # write mirrored CSVs
...
def inject_n(text: str, snippet: str, n: int, prob: float) -> str:
    """
    Split the query into words and inject ALL query words at random places
    in the passage. Each word can be injected up to `n` times (controlled
    by `prob`).
    """
    out = text

    # Split the query (translated or not) into individual words
    words = [w for w in snippet.split() if w.strip()]
    if not words:
        return out

    # For each word, try to inject it up to `n` times
    for word in words:
        for _ in range(max(0, n)):
            if rng.random() <= prob:
                out = inject_once(out, word)
