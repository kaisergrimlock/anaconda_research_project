...
TARGET_LANG = "hi"          # e.g., 'vi' for Vietnamese; 'eng'/'en' => no translation
...
OUTPUT_DIR = Path(f"retrieved/trec_dl_{TRECDL_YEAR}/{TARGET_LANG}_first/")    # write mirrored CSVs
...
def inject_n(text: str, snippet: str, n: int, prob: float) -> str:
    snippet = snippet.strip() + " "
    if not snippet or n <= 0:
        return text
    if not text:
        return snippet * n
    return snippet.strip() * n + ". " + text.strip()
