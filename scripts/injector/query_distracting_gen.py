...
DEFAULT_TREC_DL_YEAR = "2022"

LLM_COST_CSV = Path("scripts/report/llm_cost.csv")

PROMPT_INSTRUCTION = "Given the query and one or more correct answers to it, generate a sentence of distracting text that is related to the question, but does not contain the answer to the question."
...
def process_queries_blocking(
    ...
    log_root_dir = Path("logs/query_distracting") / short
    ...
    run_output_csv = output_csv.parent / f"_{run_id}_new_query_distracting.csv"
    ...
    per_run_log = log_root_dir / f"{run_id}_query_distracting_{safe_model}_trec_dl{year}.json"
    ...
    write_run_log_index(
        ...
        log_root_dir / f"{run_id}_query_distracting_logs_index_{short}_trec_dl{year}.json",
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output CSV. Default: retrieved/queries/variations/trec_dl{year}_query_distracting.csv",
    )
...
async def main():
    ...
    output_csv = args.output_csv or Path(
        f"retrieved/queries/variations/trec_dl{year}_query_distracting.csv"
    )
