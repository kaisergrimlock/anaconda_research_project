...
with open(INPUT_FILE, newline="", encoding="utf-8") as fin, \
    ...
    # Add our new columns
    if "query_vi" not in fieldnames:
        fieldnames.append("query_vi")
    ...
    for row in reader:
        ...
        query_vi = resp["TranslatedText"]

        # 2) Inject translated query into random positions in the passage
        passage = row.get("passage", "")
        passage_injected = inject_n(passage, query_vi, INJECT_COUNT, INJECT_PROB)

        # 3) Write results
        row["query_vi"] = query_vi
