...
def pick_passage_for_current_mode(row: Dict[str, str], lang: str) -> str:
    """
    Use raw passage for raw runs, otherwise use sanitized passage.
    """
    if lang == "raw":
        return (row.get("passage", "") or "").strip()
    return (row.get("sanitized", "") or "").strip()
...
def _label_single_part_file_blocking(
    ...
    required_cols = ["query"]
    required_cols.append("passage" if lang == "raw" else "sanitized")
    ...
    def worker():
        while True:
            ...
            p_for_prompt = pick_passage_for_current_mode(row_out_map, lang)
            ...
            log_obj = {
                ...
                "passage_prompt_used": "passage" if lang == "raw" else "sanitized",
