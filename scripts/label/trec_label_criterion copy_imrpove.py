...
#CRITERION_KEYS = ["coverage"]   # e.g. "exactness", "topicality", "coverage", "contextuality"..
...
def merge_one_part_into_combined(
    incoming_labels_csv: Path,
    header_out: List[str],
    combined_path: Path,
    mode: str,
) -> None:
    """
    Merge ONE per-part labels CSV into the final combined CSV immediately.

    - mode="append": append all rows.
    - mode="replace": replace rows by (qid + pid-like), append unseen keys.
      Do NOT overwrite a non-blank criterion value with blank/NaN-like.
    """
    # -----------------------------
    ...
    if "qid" not in header_out:
        raise ValueError(f"Cannot merge: missing 'qid' in header_out={header_out}")
    ...
    # -----------------------------
    # Load incoming rows (one part)
    # -----------------------------
    incoming: dict[str, list[str]] = {}
    with incoming_labels_csv.open("r", encoding="utf-8", newline="") as f_in:
        ...
        if list(in_header) != list(header_out):
            raise ValueError(
                f"Inconsistent header in {incoming_labels_csv}.\n"
                f"  got: {in_header}\n"
                f"  exp: {header_out}"
            )
        for r in reader:
    ...
    if not combined_path.exists():
        with combined_path.open("w", encoding="utf-8", newline="") as f_out:
            ...
            for r in incoming.values():
                w.writerow(r)
        print(f"[MERGE] Created combined: {combined_path} (+{len(incoming)} rows from {incoming_labels_csv.name})")
        return
    ...
    if mode == "append":
        with combined_path.open("a", encoding="utf-8", newline="") as f_out:
            ...
            for r in incoming.values():
                w.writerow(r)
        print(f"[MERGE-APPEND] {combined_path.name} += {len(incoming)} rows (from {incoming_labels_csv.name})")
        return
    ...
    with combined_path.open("r", encoding="utf-8", newline="") as f_in, tmp_path.open(
        ...
        for k, r in incoming.items():
            if k not in used_keys:
                appended_new += 1
                writer.writerow(norm_row_len(r))

    tmp_path.replace(combined_path)

    print(
        f"[MERGE-REPLACE] {combined_path.name}: replaced={replaced} kept={kept} "
        f"appended_new={appended_new} preserved_old={preserved_old} "
        f"(from {incoming_labels_csv.name})"
    )
...
async def run_for_model(model_id: str, stop_event: asyncio.Event, mode: str):
    ...
    # Final combined file path (write to this progressively)
    combined_path = MODEL_OUT_DIR / f"{short}_trecdl_{TREC_DL_YEAR}_{LANG}_{CRITERION_KEY}_labels.csv"

    # Keep per-part output files in a run folder (optional cleanup after merge)
    per_part_dir = MODEL_OUT_DIR / f"_parts_{run_id}_{model_id.replace(':','_')}_{LANG}_{CRITERION_KEY}"
    per_part_dir.mkdir(parents=True, exist_ok=True)

    sem = asyncio.Semaphore(min(6, len(part_files)))
    results: List[Dict[str, Any]] = []
    header_out_seen: Optional[List[str]] = None

    async def sem_task(p: Path):
        nonlocal header_out_seen

        async with sem:
            if stop_event.is_set():
                return None

            # 1) label this part file -> writes a per-part labels CSV
            r = await label_single_part_file(
                p, model_id, prompt_template, run_id, per_part_dir, MODEL_LOGS_DIR, stop_event
            )
            if not r:
                return None

            # 2) merge immediately into the combined output (thread-safe)
            part_labels_csv = Path(r["labels_csv"])
            part_header_out = r["header_out"]

            # ensure consistent header across parts
            if header_out_seen is None:
                header_out_seen = list(part_header_out)
            elif list(part_header_out) != list(header_out_seen):
                raise ValueError(
                    f"Inconsistent output header across parts.\n"
                    f"  first: {header_out_seen}\n"
                    f"  this : {part_header_out}\n"
                    f"  part : {p}"
                )

            with COMBINED_WRITE_LOCK:
                merge_one_part_into_combined(
                    incoming_labels_csv=part_labels_csv,
                    header_out=header_out_seen,
                    combined_path=combined_path,
                    mode=mode,
                )

            # optional: delete the per-part file after merge
            try:
                part_labels_csv.unlink(missing_ok=True)
            except Exception as e:
                print(f"[WARN] Could not delete per-part labels {part_labels_csv}: {e}")
    ...
    # optional: remove per-part folder (it should be mostly empty if we unlink per-part csvs)
    try:
        shutil.rmtree(per_part_dir, ignore_errors=False)
        print(f"[CLEANUP] Removed per-part folder: {per_part_dir}")
    except Exception as e:
        print(f"[WARN] Failed to remove per-part folder {per_part_dir}: {e}")
