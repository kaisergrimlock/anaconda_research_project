...
PROMPT_FILE = Path("prompts/remove_query_word.txt")
...
OUTPUT_COL = "passage_removed_query_word"

short = model_short_name(MODELS[0])

OUTPUT_ROOT_DIR = Path(f"retrieved/removed_query_word/trec_dl_{TREC_DL_YEAR}/{short}")
LOG_ROOT_DIR = Path("logs/removed_query_word") / short
...
def build_prompt(prompt_template: str, passage: str, query: str, row_idx: int) -> str:
    ...
    if unresolved:
        raise RuntimeError(
            ...
            f"Check that prompts/remove_query_word.txt uses single braces."
        )
...
def write_combined_csv(
    ...
    combined_path = out_dir / f"{model_short}_trecdl_{year}_{lang}_removed_query_word.csv"
...
def _process_single_part_file_blocking(
    ...
    labels_path = per_file_out_dir / f"{part_csv.stem}_removed_query_word_{safe_model}.csv"
        ...
        while True:
            ...
            try:
                ...
                if idx == 1:
                    ...
                    print(f"query_col_used={query_col_used}")
                    print(f"passage_col_used={passage_col_used}")
                    print(prompt[:1500])
            ...
            log_obj = {
                ...
                "query": query,
                "passage": passage,
                "query_col_used": query_col_used,
                "passage_col_used": passage_col_used,
                "prompt": prompt,
    ...
    per_file_log = logs_dir / f"{run_id}_removed_query_word_{safe_model}_{part_csv.stem}.json"
...
async def run_for_model_and_lang(
    ...
    write_run_log_index(
        ...
        model_logs_dir / f"{run_id}_removed_query_word_logs_index_{short}_{lang}.json",
    )
