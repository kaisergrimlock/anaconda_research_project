...
LANGS = ["eng_instruct", "ar_instruct", "vi_instruct", "th_instruct", "fr_instruct", "ru_instruct", "he_instruct", "sw_instruct", "ga_instruct", "hi_instruct", "zh_instruct"]
START_PART = 1
END_PART = 6
...
LOG_ROOT_DIR = Path("logs/instruct_removed") / short
...
def build_prompt(prompt_template: str, passage: str, row_idx: int) -> str:
    prompt = prompt_template.replace("{passage}", passage)

    unresolved = [
        token for token in ["{passage}", "{{passage}}"]
        if token in prompt
    ]
    ...
    if unresolved:
        raise RuntimeError(
            ...
            f"Check that prompts/remove_instruct.txt uses single braces."
        )

    if "{query}" in prompt or "{{query}}" in prompt:
        raise RuntimeError(
            f"Prompt still contains query placeholders at row {row_idx}. "
            "This script no longer inserts the query. Remove {query} from prompts/remove_instruct.txt."
        )
...
def _process_single_part_file_blocking(
    ...
    labels_path = per_file_out_dir / f"{part_csv.stem}_instruct_removed_{safe_model}.csv"
    ...
    def worker():
        while True:
            ...
            try:
                ...
                print(
                    f"\n[ERROR] {part_csv.name}: row={idx}, pid={pid} :: {err}"
                )
    ...
    per_file_log = logs_dir / f"{run_id}_instruct_removed_{safe_model}_{part_csv.stem}.json"
...
async def run_for_model_and_lang(
    ...
    if "{passage}" not in prompt_template:
        print("[FATAL] Prompt file must contain {passage}.")
        print(f"[PROMPT FILE] {PROMPT_FILE}")
        sys.exit(2)

    if "{query}" in prompt_template or "{{query}}" in prompt_template:
        print("[FATAL] Prompt file must not contain {query}; this script no longer inserts the query.")
        print(f"[PROMPT FILE] {PROMPT_FILE}")
        sys.exit(2)
    ...
    print(
        f"\n--- Removing injected instruction text ---\n"
        f"Model: {model_id}\n"
        f"Run ID: {run_id}\n"
        f"Lang: {lang}\n"
    ...
    write_run_log_index(
        ...
        model_logs_dir / f"{run_id}_instruct_removed_logs_index_{short}_{lang}.json",
    )
