...
# LANGS = [
#     "eng_instruct_instruct_rem", "ar_qp_rem", "vi_qp_rem", "th_qp_rem", "fr_qp_rem", "ru_qp_rem",
...
def _label_single_part_file_blocking(
    ...
    if lang == "raw":
        ...
        # For non-raw language folders, require at least one usable passage source.
        # Prefer passage_removed, but allow fallback to passage_injected.
        if "passage_removed" not in header_in and "passage_injected" not in header_in:
            print(
                f"[FATAL] {part_csv.name}: missing required passage columns. "
                "Expected at least one of ['passage_removed', 'passage_injected']."
            )
            sys.exit(2)
    ...
    def worker():
        while True:
            ...
            # Passage selection:
            # - raw uses the original passage column
            # - non-raw uses passage_removed first
            # - if passage_removed is blank/missing, fall back to passage_injected
            passage_prompt_used = "passage"
            if lang == "raw":
                p_for_prompt = (row_out_map.get("passage", "") or "").strip()
            else:
                passage_removed = (row_out_map.get("passage_removed", "") or "").strip()
                passage_injected = (row_out_map.get("passage_injected", "") or "").strip()

                if passage_removed:
                    p_for_prompt = passage_removed
                    passage_prompt_used = "passage_removed"
                else:
                    p_for_prompt = passage_injected
                    passage_prompt_used = "passage_injected_fallback"

            if not q_for_prompt or not p_for_prompt:
                print("[DEBUG ROW]", idx, row_out_map)
                print("[DEBUG passage_removed]", repr(row_out_map.get("passage_removed", "")))
                print("[DEBUG passage_injected]", repr(row_out_map.get("passage_injected", "")))
                msg = (
            ...
            log_obj = {
                ...
                "passage_prompt_used": passage_prompt_used,
