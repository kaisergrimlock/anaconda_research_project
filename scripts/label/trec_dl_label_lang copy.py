...
def _header_cols(lang_out_name: str) -> List[str]:
    # Overwrite key = (qid, pid, docid)
    return ["qid", "pid", "docid", "passage", lang_out_name, "relevance"]
...
def _label_single_part_file_blocking(
    ...
    Returns dict with: part, rows, input_tokens, output_tokens, labels_csv, log_json, lang_out_col
    """
    ...
    # Ensure per-file labels header
    if not labels_path.exists():
        with labels_path.open("w", encoding="utf-8", newline="") as f:
            csv.DictWriter(f, fieldnames=_header_cols(lang_out_col)).writeheader()
    ...
    for idx, row in enumerate(read_rows_stream(part_csv), start=1):
        ...
        try:
            ...
            with labels_path.open("a", encoding="utf-8", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=_header_cols(lang_out_col))
                writer.writerow({
                    "qid": qid,
                    "pid": pid,
                    "docid": docid,
            ...
            logs.append({
                "pid": pid, "docid": docid, "qid": qid, "query": query,
                "prompt": prompt,
                "response_text": "",
        ...
        # write labeled row (qid,pid,docid key; both passages recorded)
        with labels_path.open("a", encoding="utf-8", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=_header_cols(lang_out_col))
            writer.writerow({
                "qid": qid,
...
# Overwrite semantics by key = (qid, pid, docid)
# ----------------------------
def merge_labels(
    ...
    key_cols = ("qid", "pid", "docid")
    header = _header_cols(lang_out_name)

    rows_by_key: Dict[Tuple[str, str, str], Dict[str, str]] = {}

    # Load existing combined (if any) first
    if combined_out.exists():
        with combined_out.open("r", encoding="utf-8", newline="") as f:
            reader = csv.DictReader(f, skipinitialspace=True)
            for r in reader:
                k = tuple((r.get(c, "") or "").strip() for c in key_cols)  # type: ignore
                rows_by_key[k] = {h: (r.get(h, "") or "") for h in header}

    # Merge per-file labels (deterministic order: by filename); latest wins for identical key
    for path_str in sorted(per_file_labels):
        if stop_event is not None and stop_event.is_set():
        ...
        with p.open("r", encoding="utf-8", newline="") as in_f:
            reader = csv.DictReader(in_f, skipinitialspace=True)
            for r in reader:
                k = tuple((r.get(c, "") or "").strip() for c in key_cols)  # type: ignore
                normalized = {h: (r.get(h, "") or "") for h in header}
                rows_by_key[k] = normalized  # overwrite

    # Rewrite the combined file from scratch
    with combined_out.open("w", encoding="utf-8", newline="") as out_f:
        writer = csv.DictWriter(out_f, fieldnames=header)
        writer.writeheader()
        writer.writerows(rows_by_key.values())

    print(f"[MERGE] Wrote {len(rows_by_key)} rows (deduped by qid,pid,docid) into: {combined_out}")
...
async def run_for_model(model_id: str, stop_event: asyncio.Event):
    ...
    if results and not stop_event.is_set():
        ...
        # Merge per-file label CSVs into the per-model combined OUTPUT_FILE with overwrite semantics
