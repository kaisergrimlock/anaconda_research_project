...
TARGET_LANGS: List[str] = ["eng", "eng_brackets", "fr", "fr_brackets", "vi", "vi_brackets", "th", "th_brackets", "sw", "sw_brackets"]
...
def build_nist_zero_keyset(raw_file: Path) -> tuple[pd.DataFrame, Set[str], str, str]:
    ...
    print(f"[INFO] RAW base cohort size (NIST==0 only): {len(base_keys)}")
...
label_map = {
    ...
    "fr_brackets": "Baseline + FrBrackets",
    "eng_brackets": "Baseline + EnBrackets",
    "vi_brackets": "Baseline + ViBrackets",
    "th_brackets": "Baseline + ThBrackets",
    "sw_brackets": "Baseline + SwBrackets",
...
def load_label_distributions(label_dir: Path) -> pd.DataFrame:
    ...
    if not base_keys:
        print("[ERROR] Empty cohort after NIST==0 filtering on RAW; nothing to plot.")
        return pd.DataFrame()
    ...
    for file_path in label_dir.glob(pattern):
        ...
        # Build keys and restrict to the base cohort (NIST==0 cohort from RAW)
        ...
        if df_cohort.empty:
            print(f"[INFO] {file_path.name}: no rows matching RAW NIST==0 cohort keys; skipping.")
            continue
    ...
    if not df.empty:
        print("\n[DEBUG] Sum of props per variant (should be ~1.0 each, over RAW NIST==0 cohort):")
