...
from statsmodels.stats.multicomp import MultiComparison  # CHANGED (needed for ordering)
...
# ADDED: choose which models appear in the chart
# - None => plot all models
# - list => plot only these models, in this exact order
PLOT_MODELS: Optional[List[str]] = ["gpt-oss-20b", "qwen3-32b-v1"]  # <-- edit
...
# Output files include selected models (ADDED)
_plot_suffix = "_all_models" if not PLOT_MODELS else "_" + "-".join(PLOT_MODELS)
OUT_SIMUL_SVG = OUT_DIR / f"tukey_hsd_plot_simultaneous{_plot_suffix}_all_groups_{TREC_DL_YEAR}.svg"
OUT_SIMUL_PDF = OUT_DIR / f"tukey_hsd_plot_simultaneous{_plot_suffix}_all_groups_{TREC_DL_YEAR}.pdf"
...
# =========================
# Taxonomy ordering helpers (ADDED)
# =========================
_MULT_RE = re.compile(r"_mult(?:_\d+)?$")

def load_taxonomy_map(taxonomy_csv: Path) -> Dict[str, int]:
    """
    lang.csv format:
      lang,taxonomy
      eng, 5
    ...
    if "lang" not in df.columns or "taxonomy" not in df.columns:
        raise ValueError(
            f"{taxonomy_csv} must have columns 'lang' and 'taxonomy'. Found: {list(df.columns)}"
        )
    out: Dict[str, int] = {}
    for _, r in df.iterrows():
        k = str(r["lang"]).strip()
        v = pd.to_numeric(r["taxonomy"], errors="coerce")
        if k and pd.notnull(v):
            out[k] = int(v)
    return out
...
def normalize_lang(lang: str) -> str:
    ...
    Handles variants like:
      eng_mult, eng_mult_2, vi_first, ar_crit, etc.
    Strategy:
      - strip trailing _mult / _mult_#
      - then take base before first underscore
    """
    s = _MULT_RE.sub("", lang)
    return s.split("_", 1)[0]

def taxonomy_level(lang: str, tax_map: Dict[str, int], unknown: int = 99) -> int:
    if lang in tax_map:
        return tax_map[lang]
    base = normalize_lang(lang)
    return tax_map.get(base, unknown)
...
def main() -> None:
    ...
    # ADDED: filter models early (avoid loading unnecessary files)
    if PLOT_MODELS is not None:
        model_files = {m: model_files[m] for m in PLOT_MODELS if m in model_files}
        missing = [m for m in PLOT_MODELS if m not in model_files]
        if missing:
            print(f"[WARN] These models were requested but not found under LABEL_ROOT: {missing}")
    ...
    # =========================
    # Step 8: Run Tukey HSD (CHANGED to enforce ordering)
    # =========================
    tax_map = load_taxonomy_map(TAXONOMY_CSV)

    # model order: use PLOT_MODELS order if provided; else alphabetical
    model_order = PLOT_MODELS if PLOT_MODELS is not None else sorted(long_df["model"].unique())

    group_order: List[str] = []
    for model in model_order:
        if model not in set(long_df["model"].unique()):
            continue
        langs_for_model = sorted(
            long_df.loc[long_df["model"] == model, "lang"].unique(),
            key=lambda l: (taxonomy_level(l, tax_map), l),
        )
        for lang in langs_for_model:
            group_order.append(f"{model}{GROUP_SEP}{lang}")

    # Reverse so smaller taxonomy numbers appear lower on the plot (bottom)
    group_order = list(reversed(group_order))

    mc = MultiComparison(
        long_df["value"].to_numpy(),
        long_df["group"].to_numpy(),
