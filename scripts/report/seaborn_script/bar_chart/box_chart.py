...
OUT_BOX_SVG = OUT_DIR / f"boxplot_{METRIC}_2021_2022.svg"
OUT_BOX_PDF = OUT_DIR / f"boxplot_{METRIC}_2021_2022.pdf"
OUT_SUMMARY_CSV = OUT_DIR / f"boxplot_summary_{METRIC}_2021_2022.csv"
...
def get_language_order_by_taxonomy(df: pd.DataFrame) -> List[str]:
...
# Plotting
# =========================
def plot_grouped_boxplot(long_df: pd.DataFrame) -> None:
    lang_order = get_language_order_by_taxonomy(long_df)

    plot_df = long_df.copy()

    plot_df["lang"] = pd.Categorical(
        plot_df["lang"],
    ...
    if METRIC == "mean_diff":
        ax.set_ylabel("LLM - NIST")
        ax.set_title("Distribution of relevance inflation by language and model, 2021–2022")

    elif METRIC == "mae_4pt":
        ax.set_ylabel("Absolute error")
        ax.set_title("Distribution of absolute error by language and model, 2021–2022")

    elif METRIC == "disagree_rate":
        ax.set_ylabel("Disagreement")
        ax.set_title("Distribution of disagreement by language and model, 2021–2022")
    ...
    plt.savefig(OUT_BOX_SVG, format="svg")
    plt.savefig(OUT_BOX_PDF, format="pdf", bbox_inches="tight", pad_inches=0.02)
...
def main() -> None:
    ...
    print(f"[OK] Wrote boxplot SVG: {OUT_BOX_SVG}")
    print(f"[OK] Wrote boxplot PDF: {OUT_BOX_PDF}")
