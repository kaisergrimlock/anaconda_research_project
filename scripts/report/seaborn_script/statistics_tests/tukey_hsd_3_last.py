...
CATEGORY_SUFFIXES: Sequence[str] = ("_last",)
EXTRA_STRIP_SUFFIXES: Sequence[str] = ("_wo",)
CATEGORY_LABEL = "Random Position"
CATEGORY_MARKER = "x"
BASELINE_LABEL = "After Passage"
BASELINE_MARKER = "^"
...
def plot_simultaneous_collapsed_same_row(
    ...
    *,
    ...
    Plot Tukey simultaneous CIs so that:
      - ENG and ENG_WORD share the same row
    ...
    for model, sub in row_df.groupby("model", sort=False):
        ys = sub["y"].tolist()
        model_blocks.append((model, ys[0], ys[-1]))
