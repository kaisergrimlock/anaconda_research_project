...
FIG_ROOT = PROJECT_ROOT / "figures" / TREC_DL_YEAR / MODEL / "label3_nist"
FIG_ROOT.mkdir(parents=True, exist_ok=True)
CHART_TYPE = "lang"
GROUPED_FIG_PATH = FIG_ROOT / CHART_TYPE / f"label3overall_{MODEL}_{TREC_DL_YEAR}_{CHART_TYPE}_grouped"
STACKED_FIG_PATH = FIG_ROOT / CHART_TYPE / f"label3overall_{MODEL}_{TREC_DL_YEAR}_{CHART_TYPE}_stacked"
...
TARGET_LANGS: List[str] = ["eng", "fr", "de", "zh", "ca", "vi_corrected", "th", "ga", "sw"]
...
def build_nist_zero_keyset(raw_file: Path) -> tuple[pd.DataFrame, Set[str], str, str]:
    """
    Load the RAW file, filter to rows where:
      - gold relevance == 0   (NIST==3 ONLY)

    Return:
      (filtered_df, base_key_set, id_col_name, gold_col_name)
    ...
    if gold_col is None:
        print(f"[ERROR] RAW file {raw_file.name}: no gold relevance column ('relevance').")
    ...
    if id_col is None:
        ...
        return df_raw.iloc[0:0], set(), "", gold_col

    # Filter: NIST==3 ONLY
    df_zero = df_raw[df_raw[gold_col] == 3]

    if df_zero.empty:
        print(f"[WARN] RAW file {raw_file.name}: no rows with {gold_col}==3.")
        return df_zero, set(), id_col, gold_col

    base_keys: Set[str] = set(
        df_zero["qid"].astype(str) + "|" + df_zero[id_col].astype(str)
    )

    print(f"[INFO] RAW base cohort size (NIST==3 only): {len(base_keys)}")

    return df_zero, base_keys, id_col, gold_col
...
def load_label_distributions(label_dir: Path) -> pd.DataFrame:
    ...
    df_raw_zero, base_keys, raw_id_col, gold_col = build_nist_zero_keyset(raw_file)
    if not base_keys:
        print("[ERROR] Empty cohort after NIST==3 filtering on RAW; nothing to plot.")
        return pd.DataFrame()

    # Distribution for RAW itself (over the same cohort)
    if "llm_relevance" not in df_raw_zero.columns:
        print(f"[ERROR] RAW file {raw_file.name}: no 'llm_relevance' column.")
        return pd.DataFrame()

    df_raw_zero["llm_relevance"] = pd.to_numeric(df_raw_zero["llm_relevance"], errors="coerce")
    df_raw_zero = df_raw_zero.dropna(subset=["llm_relevance"])
    ...
    for file_path in label_dir.glob(pattern):
        ...
        # Build keys and restrict to the base cohort (NIST==3 cohort from RAW)
        ...
        if df_cohort.empty:
            print(f"[INFO] {file_path.name}: no rows matching RAW NIST==3 cohort keys; skipping.")
            continue
    ...
    if not df.empty:
        print("\n[DEBUG] Sum of props per variant (should be ~1.0 each, over RAW NIST==3 cohort):")
...
def plot_grouped_distribution(df: pd.DataFrame, title: str, out_path: Path) -> None:
    ...
    # Order: baseline first, then others alphabetically
    variants = sorted(df["variant"].unique(), key=lambda v: (v != "baseline", v))
...
def plot_stacked_distribution(df: pd.DataFrame, title: str, out_path: Path) -> None:
    ...
    # Order: baseline first, then alphabetically
    variants = sorted(pivot.index, key=lambda v: (v != "baseline", v))
...
if __name__ == "__main__":
    ...
    if df_labels.empty:
        print("[ERROR] No data found for RAW NIST=0 cohort; check LABEL_DIR and columns.")
    else:
