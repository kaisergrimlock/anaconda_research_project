...
FIG_ROOT = PROJECT_ROOT / "figures" / TREC_DL_YEAR / MODEL / "label2_nist"
FIG_ROOT.mkdir(parents=True, exist_ok=True)
CHART_TYPE = "en_mult"
GROUPED_FIG_PATH = FIG_ROOT / CHART_TYPE / f"label2overall_{MODEL}_{TREC_DL_YEAR}_{CHART_TYPE}_grouped"
STACKED_FIG_PATH = FIG_ROOT / CHART_TYPE / f"label2overall_{MODEL}_{TREC_DL_YEAR}_{CHART_TYPE}_stacked"
...
def build_nist_zero_keyset(raw_file: Path) -> tuple[pd.DataFrame, Set[str], str, str]:
    ...
    # Filter: NIST==2 ONLY
    df_zero = df_raw[df_raw[gold_col] == 2]

    if df_zero.empty:
        print(f"[WARN] RAW file {raw_file.name}: no rows with {gold_col}==2.")
    ...
    print(f"[INFO] RAW base cohort size (NIST==2 only): {len(base_keys)}")
...
def load_label_distributions(label_dir: Path) -> pd.DataFrame:
    ...
    if not base_keys:
        print("[ERROR] Empty cohort after NIST==2 filtering on RAW; nothing to plot.")
        return pd.DataFrame()
    ...
    for file_path in label_dir.glob(pattern):
        ...
        # Build keys and restrict to the base cohort (NIST==2 cohort from RAW)
        ...
        if df_cohort.empty:
            print(f"[INFO] {file_path.name}: no rows matching RAW NIST==2 cohort keys; skipping.")
            continue
    ...
    if not df.empty:
        print("\n[DEBUG] Sum of props per variant (should be ~1.0 each, over RAW NIST==2 cohort):")
